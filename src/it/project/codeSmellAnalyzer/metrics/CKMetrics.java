package it.project.codeSmellAnalyzer.metrics;
...
import it.project.codeSmellAnalyzer.beans.InstanceVariableBean;
import it.project.codeSmellAnalyzer.beans.MethodBean;
import it.project.codeSmellAnalyzer.beans.PackageBean;
import it.project.codeSmellAnalyzer.computation.FileUtility;
...
public class CKMetrics {
	...
	public static int getDIT(ClassBean cb, Vector<PackageBean> System, int inizialization){
		...
		if(DIT == 3) 
			...
			if(cb.getSuperclass() != null) {
				DIT++;
				for (PackageBean packageBean : System) {
					for (ClassBean cb2 : packageBean.getClasses()) {
	...
	public static int getNOC(ClassBean cb, Vector<PackageBean> System){

		int NOC = 0;
		for (PackageBean packageBean : System) {
			for (ClassBean c : packageBean.getClasses()) {
